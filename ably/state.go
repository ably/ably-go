...
func (q *pendingEmitter) Enqueue(msg *protocolMessage, ackCallback *msgAckCallback) {
	if len(q.queue) > 0 {
	...
	q.queue = append(q.queue, msgWithAckCallback{msg, ackCallback})
}
...
func (q *pendingEmitter) Ack(msg *protocolMessage, errInfo *ErrorInfo) {
	...
	for i, sch := range acked {
		...
		sch.ackCallback.call(result, err)
	}
}

type msgAckCallback struct {
	onAck            func(err error)
	onAckWithSerials func(serials []string, err error)
}

// call invokes the appropriate ackCallback based on which is set.
// If onAckWithSerials is set, extracts serials from res before calling.
// If onAck is set, calls it with just the error.
func (cb *msgAckCallback) call(res []*protocolPublishResult, err error) {
	if cb == nil {
		return
	}
...
func (q *msgQueue) Enqueue(msg *protocolMessage, ackCallback *msgAckCallback) {
	q.mtx.Lock()
...
func (q *msgQueue) Flush() {
	...
	for _, queueMsg := range q.queue {
		q.conn.send(queueMsg.msg, queueMsg.ackCallback)
	}
	q.queue = nil
	q.mtx.Unlock()
}
...
func (q *msgQueue) Fail(err error) {
	...
	for _, queueMsg := range q.queue {
		...
		queueMsg.ackCallback.call(nil, newError(90000, err))
	}
	q.queue = nil
	q.mtx.Unlock()
}
