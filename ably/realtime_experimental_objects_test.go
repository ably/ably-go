	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			channelMock := &channelMock{
				SendFunc: func(msg *protocolMessage, ackCallback *msgAckCallback) error {
					if tt.sendError != nil {
						return tt.sendError
					}
					...
					// Simulate async ack
					go func() {
						ackCallback.call(nil, tt.ackError)
					}()
					return nil
				},
...
func TestRealtimeExperimentalObjects_PublishObjectsContextCancellation(t *testing.T) {
	...
	channelMock := &channelMock{
		SendFunc: func(msg *protocolMessage, ackCallback *msgAckCallback) error {
			// Don't call ackCallback to simulate hanging
			return nil
		},
		GetClientOptionsFunc: func() *clientOptions {
			return &clientOptions{
...
func (c channelMock) send(msg *protocolMessage, ackCallback *msgAckCallback) error {
	return c.SendFunc(msg, ackCallback)
}
