...
func TestPendingEmitter_AckResult(t *testing.T) {
	...
	t.Run("three messages", func(t *testing.T) {
		...
		callback1 := &msgAckCallback{onAckWithSerials: func(serials []string, err error) { msg1Serials = serials }}

		protoMsg2 := &protocolMessage{MsgSerial: 2, Action: actionMessage, Channel: "test"}
		callback2 := &msgAckCallback{onAckWithSerials: func(serials []string, err error) { msg2Serials = serials }}

		protoMsg3 := &protocolMessage{MsgSerial: 3, Action: actionMessage, Channel: "test"}
		callback3 := &msgAckCallback{onAckWithSerials: func(serials []string, err error) { msg3Serials = serials }}
