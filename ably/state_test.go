...
func emit(serial int64, count int, fn func(*pendingEmitter, int64, int, *ErrorInfo), err *ErrorInfo) func(*pendingEmitter) {
	return func(q *pendingEmitter) {
		fn(q, serial, count, err)
	}
}
