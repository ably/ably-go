...
func (c *RealtimeChannel) PublishMultipleAsync(messages []*Message, onAck func(err error)) error {
	...
	return c.send(msg, &msgAckCallback{onAck: onAck})
}

// PublishWithResult publishes a single message to the channel and returns the serial assigned
// by the server (RTL6j, RTL6j1 alternative).
// This will block until either the publish is acknowledged or fails to deliver.
func (c *RealtimeChannel) PublishWithResult(ctx context.Context, name string, data interface{}) (*PublishResult, error) {
	results, err := c.PublishMultipleWithResult(ctx, []*Message{{Name: name, Data: data}})
	if err != nil {
		return nil, err
	}
	if len(results) == 0 {
...
// PublishMultipleWithResult publishes all given messages on the channel and returns the serials
// assigned by the server (RTL6j, RTL6j1 alternative).
func (c *RealtimeChannel) PublishMultipleWithResult(ctx context.Context, messages []*Message) ([]PublishResult, error) {
	type resultOrError struct {
		results []PublishResult
		err     error
	}
	listen := make(chan resultOrError, 1)
	onAck := func(results []PublishResult, err error) {
		listen <- resultOrError{results, err}
	}
	if err := c.PublishMultipleWithResultAsync(messages, onAck); err != nil {
		return nil, err
	}
...
func (c *RealtimeChannel) PublishMultipleWithResultAsync(messages []*Message, onAck func([]PublishResult, error)) error {
	...
	return c.sendWithSerialCallback(msg, func(serials []string, err error) {
		if err != nil {
			onAck(nil, err)
			return
		}
...
func (c *RealtimeChannel) performMessageOperationAsync(msg *Message, action MessageAction, onAck func(*UpdateDeleteResult, error), options ...UpdateOption) error {
	...
	id := c.client.Auth.clientIDForCheck()
	if msg.ClientID != "" && id != wildcardClientID && msg.ClientID != id {
		// Spec RTL6g3,RTL6g4
		return fmt.Errorf("Unable to publish message containing a clientId (%s) that is incompatible with the library clientId (%s)", msg.ClientID, id)
	}
	...
	// RTL32c: Copy message to avoid mutating user-supplied object.
	opMsg := *msg
	// RTL32b1: Set action for the operation.
	opMsg.Action = action
	// RTL32b2: Set version from user-supplied operation metadata.
	opMsg.Version = opts.version

	// RTL32d (encoding): Encode message data per RSL4/RSC8.
	cipher, _ := (*protoChannelOptions)(c.options).GetCipher()
	opMsg, err := opMsg.withEncodedData(cipher)
	if err != nil {
		return fmt.Errorf("encoding data for message: %w", err)
	}

	// RTL32b: Send MESSAGE ProtocolMessage with single Message.
	protoMsg := &protocolMessage{
		Action:   actionMessage,
		Channel:  c.Name,
		Messages: []*Message{&opMsg},
		Params:   opts.params, // RTL32e: Pass params in ProtocolMessage.params.
	}
	...
	return c.sendWithSerialCallback(protoMsg, func(serials []string, err error) {
		...
		// RTL32d: Extract versionSerial from first element of ACK serials.
...
// GetMessage retrieves a message by serial, delegates to REST (RTL28, RSL11).
func (c *RealtimeChannel) GetMessage(ctx context.Context, serial string) (*Message, error) {
	return c.client.rest.Channels.Get(c.Name).GetMessage(ctx, serial)
}

// GetMessageVersions retrieves the version history of a message by serial, delegates to REST (RTL31, RSL14).
func (c *RealtimeChannel) GetMessageVersions(serial string, params url.Values) HistoryRequest {
	return c.client.rest.Channels.Get(c.Name).GetMessageVersions(serial, params)
}

func (c *RealtimeChannel) send(msg *protocolMessage, ackCallback *msgAckCallback) error {
	if enqueued := c.maybeEnqueue(msg, ackCallback); enqueued {
		return nil
	}
	...
	c.client.Connection.send(msg, ackCallback)
	return nil
}

// sendWithSerialCallback sends a message and calls onAck with serials extracted from ACK.
func (c *RealtimeChannel) sendWithSerialCallback(msg *protocolMessage, onAck func(serials []string, err error)) error {
	return c.send(msg, &msgAckCallback{onAckWithSerials: onAck})
}

func (c *RealtimeChannel) maybeEnqueue(msg *protocolMessage, ackCallback *msgAckCallback) bool {
	// RTL6c2
