...
func (pres *RealtimePresence) maybeEnqueue(msg *protocolMessage, callback *ackCallback) bool {
	if pres.channel.opts().NoQueueing {
		if callback != nil {
			callback.call(nil, errors.New("unable enqueue message because Options.QueueMessages is set to false"))
		}
		return false
	}
	pres.queue.Enqueue(msg, callback)
	return true
}
...
func (pres *RealtimePresence) send(msg *PresenceMessage) (result, error) {
	...
	callback := &ackCallback{onAck: onAck}
	switch pres.channel.State() {
	case ChannelStateInitialized: // RTP16b
		if pres.maybeEnqueue(protomsg, callback) {
			pres.channel.attach()
		}
	case ChannelStateAttaching: // RTP16b
		pres.maybeEnqueue(protomsg, callback)
	case ChannelStateAttached: // RTP16a
		pres.channel.client.Connection.send(protomsg, callback) // RTP16a, RTL6c
	}
