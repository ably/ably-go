...
//updateInnerMessagesEmptyFields - Update inner Message empty Id, connectionId and Timestamp (TM2a, TM2c, TM2f)
func (p protocolMessage) updateInnerMessagesEmptyFields() {
	for msgIndex, message := range p.Messages {
		message.updateEmptyFields(p, msgIndex)
	}
	for presenceMsgIndex, presenceMsg := range p.Presence {
		presenceMsg.updateEmptyFields(p, presenceMsgIndex)
	}
}
